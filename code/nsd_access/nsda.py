...
class NSDAccess(object):
    ...
    def read_image_coco_info_Img(self, image_index, info_type = 'instances'):
        if not hasattr(self, 'stim_descriptions'):
            self.stim_descriptions = pd.read_csv(
                self.stimuli_description_file, index_col=0)
        if len(image_index) == 1:
...
#                 print('annotations file not found')
            ...
            coco = COCO(annot_file)
            coco_annot_IDs = coco.getImgIds([subj_info['cocoId']])
            coco_annot = coco.loadImgs(coco_annot_IDs)
            ...
            for image in image_index:
                ...
                if subj_info['cocoSplit'] == 'train2017':
                    coco_annot_IDs = coco_train.getImgIds(
                        [subj_info['cocoId']])
                    coco_ann = coco_train.loadImgs(coco_annot_IDs)
                    coco_annot.append(coco_ann)

                elif subj_info['cocoSplit'] == 'val2017':
                    coco_annot_IDs = coco_val.getImgIds(
                        [subj_info['cocoId']])
                    coco_ann = coco_val.loadImgs(coco_annot_IDs)
                    coco_annot.append(coco_ann)
        return coco_annot
