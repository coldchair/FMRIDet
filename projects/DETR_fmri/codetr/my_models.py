...
class SinePositionalEncoding1D(BaseModule):
    ...
    def forward(self, mask: Tensor, input: Optional[Tensor] = None) -> Tensor:
        ...
        if mask is not None:
            ...
            B, _, L = input.shape
            device = input.device
            # y_embed = torch.arange(1, L + 1, dtype=torch.float32, device=device).view(1, -1).repeat(B, 1)
            x_embed = torch.arange(1, L + 1, dtype=torch.float32, device=device).view(1, -1).repeat(B, 1)

        if self.normalize:
            # y_embed = (y_embed + self.offset) / (y_embed[:, :] + self.eps) * self.scale
        ...
        # pos_x = torch.stack((pos_x[:, :, 0::2].sin(), pos_x[:, :, 1::2].cos()), dim=3).view(B, L, -1)
        # pos_y = torch.stack((pos_y[:, :, 0::2].sin(), pos_y[:, :, 1::2].cos()), dim=3).view(B, L, -1)
